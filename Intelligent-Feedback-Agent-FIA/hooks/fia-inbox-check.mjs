#!/usr/bin/env node
// FIA owner nudge — sessionStart hook. Registered from BOTH
// .cursor/hooks.json and .claude/settings.json (Step 6a/6b above) — this one
// script serves both editors; it does not need to know which one invoked it.
//
...
//
// Delivery differs by editor because their hook contracts differ:
// - Cursor: sessionStart's additional_context is confirmed dropped on Cursor
//   3.x, so the reliable channel is a generated, gitignored always-apply rules
//   file (.cursor/rules/fia-inbox.generated.mdc) — Claude Code ignores this file.
// - Claude Code: SessionStart hooks reliably deliver
//   hookSpecificOutput.additionalContext into context (no equivalent bug), so
//   that field is this script's REAL Claude Code delivery — no generated-file
//   workaround is needed on that side.
// Both fields are emitted every run regardless of which editor is actually
// running — whichever one doesn't apply is simply ignored.
//
...
async function main() {
  ...
  const body = [
    ...
    ...(kitOutdated ? [
      'FIA: your fia-feedback Skill kit is out of date',
      '',
      changelog ? `What's new: ${changelog}` : 'A newer version is available.',
      '',
      'Run the `fia-feedback` install prompt again (from your FIA dashboard Settings',
      'page) to pick up the update, then commit and push it for your users.',
      '',
    ] : []),
  ...
  // Cursor delivery: the generated always-apply rules file — reliable there
  // (see the sessionStart additional_context bug noted at the top of this file).
  const banner = [
    '---',
  ...
  // Claude Code delivery: hookSpecificOutput.additionalContext is this editor's
  // REAL nudge channel (no generated-file workaround needed — see top-of-file
  // comment). additional_context (top-level) is kept only for Cursor forward-
  // compat in case that bug is ever fixed; it's a harmless no-op today.
  process.stdout.write(JSON.stringify({
    additional_context: bodyText,
    hookSpecificOutput: { hookEventName: 'SessionStart', additionalContext: bodyText },
  }));
  process.exit(0);
}
